...
export default async function DiagnosticsPage() {
  ...
  return <div className="space-y-5 p-6"><div><Link href="/settings" className="text-sm link-action">← Настройки</Link><h1 className="mt-2 text-3xl font-bold">Диагностика</h1><p className="text-sm text-gray-500">Локальная информация для проверки состояния установки. Данные никуда не отправляются.</p></div>
    <section className="grid gap-3 rounded-xl border bg-white p-5 sm:grid-cols-2 lg:grid-cols-3">{[
      ['Версия', data.productVersion], ['ID установки', data.installationId ?? 'Недоступен'], ['Runtime', data.appRuntime], ['База данных', data.databaseStatus], ['PostgreSQL', data.postgresMajor], ['Текущая migration', data.currentMigration ?? 'Нет'], ['Точек восстановления', String(data.snapshotCount)], ['Общий размер', size(data.snapshotStorageSize)], ['PASKO Reference', data.referenceProfileStatus], ['Организация', context.organizationName], ['Команда', context.teamName], ['Сезон', context.seasonName],
