...
export type TestInit = {
  id?: string; code: string; name: string; unit: string; direction: string;
  categoryId: string | null; qcMin: number | null; qcMax: number | null;
  qcDescription: string | null; changeThreshold: number | null; cv: number | null;
  equipment: string | null; source: string | null; comment: string | null;
...
export function ArchiveButton() {
  return (
    <button type="submit" className="rounded-lg border border-red-300 bg-red-50 px-4 py-2 text-sm font-semibold text-red-700 hover:bg-red-100"
      onClick={(e) => { const ok = window.prompt('Архивировать тест? Для подтверждения введите АРХИВ:'); if (ok !== 'АРХИВ') e.preventDefault(); }}>
      Архивировать
    </button>
  );
}
export default function TestEditor({ test, categories, hasResults, resultsCount }: { test: TestInit | null; categories: Cat[]; hasResults: boolean; resultsCount: number }) {
  const router = useRouter();
  const [state, formAction] = useFormState(test?.id ? updateTest : createTest, null);
  const [rules, setRules] = useState<string[]>(test?.protocol.rules.length ? test.protocol.rules : ['']);
  const [catId, setCatId] = useState(test?.categoryId ?? '');
  useEffect(() => { if (state?.ok) router.push('/tests'); }, [state, router]);
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    if (test?.id && hasResults) {
      ...
      if (unit !== test.unit || direction !== test.direction) {
        const ok = window.confirm(`Для этого теста уже существует ${resultsCount} результатов. Продолжить?`);
    ...
    <form action={formAction} onSubmit={onSubmit} className="space-y-5">
      ...
      <div className={section}>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          {catId === '__new__' && <label className={label}>Название новой категории<input name="newCategory" className={field} placeholder="Плиометрика" /></label>}
          <label className={label}>Единица измерения *
            <input name="unit" required list="units-list" defaultValue={test?.unit ?? ''} placeholder="см" className={field} />
            <datalist id="units-list">
              {['см', 'м', 'с', 'мс', 'кг', '%', 'Вт', 'Вт/кг', 'м/с', 'баллы', 'км/ч'].map((u) => <option key={u} value={u} />)}
            </datalist>
          </label>
        </div>
      </div>
      <div className={section}>
        ...
        <label className={label}>Направление
          <select name="direction" defaultValue={test?.direction ?? 'HIGHER_IS_BETTER'} className={field}>
      ...
      <div className={section}>
        ...
        <div className="space-y-4">
          <label className={label}>Как выполнять<textarea name="how" rows={4} defaultValue={test?.protocol.how ?? ''} className={field} /></label>
          <label className={label}>Как определяется результат<input name="result" defaultValue={test?.protocol.result ?? ''} className={field} /></label>
          <div>
            <span className={label}>Критерии зачёта</span>
            <div className="mt-1 space-y-2">
              {rules.map((r, i) => (
                <div key={i} className="flex gap-2">
                  <input name="rules" value={r} onChange={(e) => setRules((rs) => rs.map((x, j) => j === i ? e.target.value : x))} placeholder={`Правило ${i + 1}`} className={field} />
                  <button type="button" onClick={() => setRules((rs) => rs.filter((_, j) => j !== i))} className="shrink-0 rounded-lg border border-gray-200 px-3 text-sm text-gray-500 hover:bg-gray-50">✕</button>
                </div>
              ))}
            </div>
            <button type="button" onClick={() => setRules((rs) => [...rs, ''])} className="mt-2 text-sm font-medium hover:underline" style={{ color: 'var(--red)' }}>+ Добавить правило</button>
          </div>
        </div>
      </div>
      <div className={section}>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <label className={label}>Оборудование<input name="equipment" defaultValue={test?.equipment ?? ''} className={field} /></label>
          <label className={label}>Источник / автор<input name="source" defaultValue={test?.source ?? ''} className={field} /></label>
          <label className={`${label} md:col-span-2`}>Комментарий<textarea name="comment" rows={2} defaultValue={test?.comment ?? ''} className={field} /></label>
        </div>
      </div>
      <div className="flex flex-wrap items-center gap-3">
        <button className="btn-primary">Сохранить тест</button>
        <a href="/tests" className="rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">Отмена</a>
