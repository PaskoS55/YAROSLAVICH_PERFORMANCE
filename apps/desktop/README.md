...
The Windows package includes the complete EDB PostgreSQL 16.14 x64 binary archive as `resources/postgres`, outside ASAR. The pinned source and SHA-256 are recorded in `postgres-runtime.json`; EDB does not publish an independent checksum beside this ZIP, so the pinned digest was established from the initial trusted HTTPS download. The same manifest pins Microsoft Visual C++ x64 Runtime 14.51.36247.0 and the three app-local DLLs required by PostgreSQL's PE dependency chain. They are reproducibly extracted from the official Microsoft Redistributable during preparation, placed beside the PostgreSQL executables, and verified by checksum and static import closure. No PostgreSQL or VC++ system installer, Windows service, administrator permission, or customer-time download is used.
