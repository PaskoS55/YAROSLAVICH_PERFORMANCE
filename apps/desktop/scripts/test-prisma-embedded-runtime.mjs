...
try {
  ...
  if (fresh !== '7|16|0|0|0|4') throw new Error(`Unexpected fresh state: ${fresh}`);
  ...
  if (await executeSql({ runtime: postgres, username: APPLICATION_USER, password: credentials.applicationPassword, database: 'pasko_performance', sql: 'SELECT (SELECT count(*) FROM test_categories),(SELECT count(*) FROM tests),(SELECT count(*) FROM _prisma_migrations);' }) !== '7|16|4') throw new Error('Reference bootstrap is not idempotent');
  ...
  if (preserved !== '1|1|4|1') throw new Error(`Upgrade did not preserve data/schema: ${preserved}`);
