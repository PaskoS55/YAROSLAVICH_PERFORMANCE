...
try {
  ...
  await executeSql({ runtime: postgres, username: APPLICATION_USER, password: credentials.applicationPassword, database: 'p5_upgrade', sql: `INSERT INTO organizations(id,name,code,"createdAt","updatedAt") VALUES ('old_org','Old Club','OLD',now(),now()); INSERT INTO teams(id,name,code,"organizationId","createdAt","updatedAt") VALUES ('old_team','Old Team','MAIN','old_org',now(),now()); INSERT INTO test_categories(id,code,name,"sortOrder",active,"includeInRadar","createdAt","updatedAt") VALUES ('cat','LEGACY','Legacy',1,true,false,now(),now()); INSERT INTO tests(id,code,name,"categoryId","isSystem",direction,unit,"createdAt","updatedAt") VALUES ('test_a','OLD_A','Old A','cat',false,'HIGHER_IS_BETTER','cm',now(),now()),('test_b','OLD_B','Old B','cat',false,'LOWER_IS_BETTER','sec',now(),now()); INSERT INTO norms(id,"testCode",position,"anchor10","anchor25","anchor50","anchor75","anchor90",source,"validFrom","validUntil","createdAt","updatedAt","testId") VALUES ('norm_a','OLD_A','setter',1,2,3,4,5,'source-a','2020-01-01',NULL,now(),now(),'test_a'),('norm_b','OLD_B','libero',9,8,7,6,5,'source-b','2021-01-01','2025-01-01',now(),now(),NULL);` });
  ...
  if (preserved !== '1|1|2|5') throw new Error(`Upgrade did not preserve data/schema: ${preserved}`);
  const legacy = await executeSql({ runtime: postgres, username: APPLICATION_USER, password: credentials.applicationPassword, database: 'p5_upgrade', sql: `SELECT string_agg(t.code||':'||e.position||':'||e.p10||':'||e.p25||':'||e.p50||':'||e.p75||':'||e.p90||':'||e.source_text,',' ORDER BY t.code) FROM norm_entries e JOIN tests t ON t.id=e.test_id WHERE e.profile_id='legacy-imported-profile';` });
  if (legacy !== 'OLD_A:setter:1:2:3:4:5:source-a,OLD_B:libero:9:8:7:6:5:source-b') throw new Error(`Legacy values/provenance changed: ${legacy}`);
  ...
  console.log('Packaged Prisma embedded integration PASS: fresh PASKO Reference bootstrap, idempotence, previous-schema upgrade, exact legacy Norm migration, broken migration fail-closed');
